...
The same reasoning extends past the crate's own files to what it links against. Dependency versions change blob bytes — `serde_json`'s `preserve_order` decides whether every `Value::Object` in an option value keeps Nix's key order or gets re-sorted, and the `tree-sitter-bash` grammar version sets the token runs baked into every `DrvPhase` — so `build.rs` also hashes the extraction crate's **resolved dependency closure**, walked out of `Cargo.lock`. Deliberately the closure and not the whole lock: the workspace's other member pulls `axum`, `tower`, `notify`, `futures` and `tokio-stream`, and with dependabot running, hashing the whole lock would let an `axum` bump throw away every user's cached extraction — the same over-invalidation the crate split removed, re-entered one level up. Feature selections are not in the lock, so the two manifests that can declare one are hashed too: the extraction crate's `Cargo.toml` in full, and from the workspace root only the `[workspace.dependencies]` entries naming a package in the closure. Every fallback in that code hashes more rather than less, because being coarse costs a re-extraction and being silently narrow serves stale data.

A boundary that is only a property of where files sit is checked by nobody, so there is one test on it: [`tests/determinism.rs`](../tests/determinism.rs) fails if the root crate grows a new file-writing site. That is the mechanism by which root-crate code could start putting bytes on disk that the fingerprint does not describe — the root crate depends on the extraction crate, so every schema type is already in scope and a new `serve.rs` helper could build a `ConfigData` and write it out, compiling cleanly. The test's own comment is explicit about what it cannot catch; it is a prompt to think rather than a proof, and its allowlist going stale fails loudly rather than quietly.
...
Because that pool completes chunks in whatever order it finishes them, the collected options are **sorted by `loc`** before entries are built. Without the sort the blob's option order — and the `fileIndex` key order and every `defines`/`declares` index derived from it — depended on scheduling and on the machine's core count: ten runs of one binary against the fixture produced five distinct blob hashes, so the same flake cached different bytes on a 4-core and a 16-core host. `loc` is the option's path and therefore its identity, so it is a total order on real data, and nothing consumes the array order as meaning (the SPA reaches options through `fileIndex` and its own loc→index map). [`tests/determinism.rs`](../tests/determinism.rs) holds this: repeated extractions must be byte-identical. The sidecar is still not byte-stable, since it records `extractedAt` and `durationMs`.
