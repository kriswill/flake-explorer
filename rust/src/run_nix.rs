// Thin wrapper around the host's `nix` binary — port of src/extract/run-nix.ts.
...
pub const EXTRACT_NIX: &str = include_str!("../../src/extract/extract.nix");

#[derive(Debug)]
pub struct NixError {
    pub message: String,
    pub stderr: String,
    #[allow(dead_code)] // part of the error contract, mirrors run-nix.ts
